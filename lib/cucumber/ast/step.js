function Step(data, uri, previousStep) {
  ...
  if (data.argument){
    if (data.argument.type === 'DataTable') {
      dataTable = Cucumber.Ast.DataTable(data.argument);
    } else if (data.argument.type === 'DocString') {
      docString = Cucumber.Ast.DocString(data.argument);
    }
  }
